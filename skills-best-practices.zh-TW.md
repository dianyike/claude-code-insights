...
>
> 基於 Anthropic 官方文件、熱門 GitHub 儲存庫、社群經驗及 [Lessons from Building Claude Code: How We Use Skills](https://www.linkedin.com/pulse/lessons-from-building-claude-code-how-we-use-skills-thariq-shihipar-iclmc) 整理
...
6. [Skill 類型與呼叫控制](#6-skill-類型與呼叫控制)
...
Skill 是由**指令、腳本與資源**組成的結構化資料夾，Claude 能夠動態發現並載入這些內容，以提升特定任務的表現。
...
- Claude 根據 `description` 自動判斷是否載入，或由使用者透過 `/skill-name` 手動觸發
...
| **可重複使用** | 同一 skill 可跨專案（`~/.claude/skills/`）或跨組織（enterprise）共用 |
| **支援 Hooks** | 可在 skill 生命週期中註冊動態 hooks |
| **靈活設定** | 工具白名單、模型選擇、effort 等級、subagent 隔離 |
| **目錄結構** | 善用子目錄放置模板、範例、腳本與參考文件 |

### Skill 的目標

**封裝可移植、可重複使用的過程知識。** `name` 和 `description` 必定載入 context（用於發現），完整指令僅在觸發時載入（節省 context）。Skill 可以讀取腳本，也可以執行腳本。
...
- **第三層延遲載入**：大型 API 文件、範例集不必每次都載入

### Context 預算

Skill descriptions 的總量上限會動態計算為 context window 的 2%，fallback 為 16,000 字元。超過時部分 skill 會被排除。可用 `/context` 檢查，或設定環境變數 `SLASH_COMMAND_TOOL_CHAR_BUDGET` 覆蓋。
...
**Task Content（任務型）**：給 Claude 具體的步驟化指令。通常搭配 `disable-model-invocation: true`，由使用者手動觸發。
...
Claude 已經掌握大量通用程式設計知識。Skill 不該重複教材，而該補充 Claude **預設不知道**的內容：
...
| 踩過的坑和變通作法 | 官方文件已涵蓋的內容 |

### 4.3 建立 Gotchas 區段

任何 Skill 裡**訊號最強**的，往往不是教學內容，而是 Gotchas（踩坑記錄）。因為只有你們團隊真正遇過的問題才是獨特知識。

```markdown
## Gotchas

- `api/v2/users` 返回的 `created_at` 是 UTC，但 `updated_at` 是本地時區（已提 issue，短期內不會修）
- 測試環境的 rate limit 是 10 req/s，不是文件寫的 100，批次測試必須加上延遲
...
| Frontmatter 設定 | 使用者可觸發 | Claude 可觸發 | Context 載入行為 |
|------------------|-----------|--------------|-----------------|
...
| **僅使用者觸發** | `disable-model-invocation: true` | 有副作用的操作：deploy、commit、send message |
...
| 位置 | 路徑 | 適用範圍 |
|------|------|---------|
| Enterprise | 透過 managed settings 部署 | 組織內所有使用者 |
...
**注意**：`context: fork` 只適用於有明確任務指令的 skill。如果 skill 只包含指南（如「使用這些 API 慣例」）而沒有具體任務，subagent 收到指南但沒有可執行的 prompt，會徒然返回。

### 8.3 儲存腳本與視覺化輸出

**核心原則**：把穩定的能力封裝成腳本和輔助函式，讓 Claude 負責**組合**而非重造輪子。別讓 Claude 每次都從頭寫樣板程式碼，給它程式碼，不給它文件。
...
這個模式適用於：依賴圖、測試覆蓋報告、API 文件、資料庫 schema 視覺化等。
...
Skill 每次執行都是無狀態的。用 `config.json` 儲存使用者偏好，讓 Skill 從**無狀態工具**變成**有狀態助手**：
...
Skill 不記得上次做了什麼。解決方案是讓 Skill 寫日誌，下次執行時讀取日誌，只報告**增量變化**：
...
Skill 內容會成為 subagent 的任務 prompt，由 `agent` 欄位決定執行環境：
...
> **WARNING**：`context: fork` 只適用於有**明確任務指令**的 skill。如果 skill 只包含指南（如「使用這些 API 慣例」）而沒有具體任務，subagent 收到指南但沒有可執行的 prompt，會徒然返回。在「方向一」中使用 `context: fork` 時，SKILL.md 正文**必須包含可執行的任務描述**，不能只有參考知識。
...
| 函式庫用法範例 | MCP（如 Context7） | 即時取得最新文件 |
...
3. description 包含使用者可能說的關鍵字，並以動詞開頭
...
| 問題 | 根因 | 解法 |
|------|------|------|
| Skill 不觸發 | description 模糊或關鍵字不匹配 | 重寫 description，包含使用者自然會說的詞。用 `/skill-name` 確認 skill 存在 |
...
- [Skill Issue: Harness Engineering for Coding Agents (HumanLayer)](https://www.humanlayer.dev/blog/skill-issue-harness-engineering-for-coding-agents) — Skills 作為漸進式披露的實踐案例，含 context 腐蝕研究資料
...
- [ ] 需要使用者偏好時，用 `config.json` 儲存設定（§8.5）
