...
4. [該寫什麼](#4-該寫什麼)
5. [不該寫什麼](#5-不該寫什麼)
6. [學術研究：context 檔案的實際效果](#6-學術研究context-檔案的實際效果)
...
CLAUDE.md 是 Claude Code 的**專案 context 檔案**，告訴 Claude「這個專案是什麼、怎麼跑、有什麼規則」。它在**每次對話開始時自動載入**，相當於你每次都在對話開頭貼上的系統提示。
...
Claude Code 提供了 `/init` 指令，讓 Claude 分析你的 codebase 後自動產生 CLAUDE.md。聽起來很方便，但根據研究（見[第 6 節](#6-學術研究context-檔案的實際效果)），**LLM 自動產生的 context 檔案會降低任務成功率 2-3%、增加推理成本 20%+**。

原因很簡單：`/init` 產生的 200 行裡，可能只有 10 行是 Claude 真正無法從程式碼推斷的。剩下的 190 行都是冗餘，Claude 看 `package.json`、檔案結構和程式碼本身就能知道這些資訊。這些冗餘不僅浪費 context window，還會干擾 Claude 的判斷。
...
| 方式 | 第一天就有 | 精準度 | 長期維護 |
|------|----------|--------|---------|
| `/init` 自動產生 | 200 行 | 低（90% 冗餘） | 難（不知道哪些有用） |
...
為什麼不要？這些資訊應該放在程式碼註解或獨立文件中，由 Claude 按需讀取。每次對話都載入 50 行 API 文件是浪費。

---

## 6. 學術研究：context 檔案的實際效果

### 論文概要

[Evaluating AGENTS.md](https://arxiv.org/html/2602.11988v1)（2026）首次以實證方式評估了 context 檔案對 coding agents 的影響。結果令人意外：

| 設定 | 任務成功率變化 | 推理成本變化 |
|------|--------------|-------------|
| LLM 自動產生的 context 檔案 | **下降 2-3%** | **增加 20-23%** |
| 開發者手寫的 context 檔案 | 提升約 4% | 增加約 19% |
| 無 context 檔案（基準） | — | — |

### 為什麼會有害？

1. **資訊冗餘**：LLM 產生的檔案與既有文件（README、docstring）高度重複，沒有新價值
2. **干擾導航**：程式碼庫概述並沒有幫助 agent 更快找到相關檔案，反而多走了 2-4 步
3. **推理成本暴增**：額外的 context 讓模型需要處理更多 tokens，但品質沒有提升

### 唯一有幫助的情境

當專案**完全沒有其他文件**（移除所有 markdown 文件）時，LLM 產生的 context 檔案才有正面效果（+2.7%），因為它成為唯一的文件來源。

### 對 CLAUDE.md 的啟示

| 做法 | 效果 |
|------|------|
| 用 LLM 自動產生 CLAUDE.md | 大概率有害，避免 |
...
```markdown
<!-- 反模式：讓 Claude 自己產生 CLAUDE.md -->
"Claude，幫我分析這個 codebase 然後產生 CLAUDE.md"
```

**問題**：研究顯示 LLM 產生的 context 檔案會降低 2-3% 成功率、增加 20%+ 成本。產生的內容大多是 Claude 已經能從程式碼推斷的資訊。
...
| **會話日誌分析** | 全面 | 資訊過載 |
| **claude-reflect** | 自動捕捉修正，經審查後寫入 | 需要額外設定 |
...
3. 考慮將參考文件封裝成 Claude Skills，利用 `description` 自動觸發
...
- [Claude Code 官方文件：Memory](https://docs.anthropic.com/en/docs/claude-code/memory)
...
- [Skills 最佳實踐指南](./skills-best-practices.zh-TW.md)
- [Subagent 最佳實踐指南](./subagent-best-practices.zh-TW.md)
...
- [ ] **人工維護**：CLAUDE.md 由人工撰寫和更新，不是由 Claude 自動產生
